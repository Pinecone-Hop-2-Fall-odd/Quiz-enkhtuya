...
const Quiz = ({ searchParams }) => {
  ...
  //array[quizId - 1]  | array.filter((el) => el._id === quizId)
  ...
  useEffect( () => {
    const fetchData = async () => {
      ...
      .then(res => {const quizData = res.data.data.filter((el) => el._id === quizId) ;
        setQuestionsAndAnswers(
  ...
  return (
    <div className="flex w-full h-screen justify-center bg-[#f6f7fb] p-[5%] overflow-auto text-[#000000]">
      <div className="flex flex-col items-center p-[15px] gap-[20px] ">
        ...
        {showResult && !warning ? <h1>Your score: {score}/7</h1> : ""}
        {!showResult ? (<button onClick={() => checkAnswers()} className="bg-[#2475B7] p-[10px] text-[#FFFFFF]">
            Submit</button>) : (<button onClick={() => again()} className="bg-[#2475B7] p-[10px] text-[#FFFFFF]">again..</button>)}
      </div>
    </div>
  );
};
